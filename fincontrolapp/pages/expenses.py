...
CATEGORY_ICONS = {
    ...
    "Транспорт": (ft.Icons.DIRECTIONS_CAR, "#483EB7"),
    "Здоровье": (ft.Icons.LOCAL_HOSPITAL, "#C80D00"),
...
class ExpensesPage(BasePage):
    ...
    def build_body(self):
        ...
        return ft.Column([
            ft.Text("Категории", size=16,font_family="Montserrat SemiBold",
    ...
    def _expense_list(self, expenses):
        ...
        for t in expenses:
            icon, color = CATEGORY_ICONS.get(
                t["category_name"], (ft.Icons.MORE_HORIZ, "#483EB7")
            )
    ...
    def _confirm_delete(self, transaction_id, category_name):
        ...
        def on_confirm(e):
            ...
            try:
                ...
                self.refresh()
                self._show_success("Расход удалён")
            except Exception:
                self._show_error("Не удалось удалить расход")
    ...
    def _open_add_dialog(self, e):
        ...
        desc_field = ft.TextField(
            label="Описание (необязательно)", text_style=ft.TextStyle(font_family="Montserrat SemiBold", size=15),
            border_color="#6976EB",
        )
        ...
        date_field = ft.TextField(
            ...
            read_only=True, text_style=ft.TextStyle(font_family="Montserrat SemiBold", size=15),
            border_color="#6976EB",
    ...
    def _open_edit_dialog(self, transaction):
        ...
        category_dd = ft.Dropdown(
            label="Категория", text_style=ft.TextStyle(font_family="Montserrat SemiBold", size=15),
            border_color="#6976EB",
            options=[ft.dropdown.Option(str(c.id), c.name) for c in cats],
        ...
        def on_submit(e):
            ...
            try:
                ...
                self._show_error("Не удалось сохранить расход", close_bs=bs)
                return
            self.rebuild()
            pages = self.page_ref.data.get("pages", {})
            if 0 in pages:
            ...
            bs.open = False
            self.page.update()
            self._show_success("Расход сохранён")
